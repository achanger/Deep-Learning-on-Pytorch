...
# Training model by just running this script file.
# All saved checkpoints will be saved in Folder <checkpoints>

import os

# Step 1. load parameters.
from config import get_config
config = get_config("./config/configFiles/config_demo.json")
config.root_dir = os.getcwd()

quit()
# Step 2. load datasets.
from dataLoader import get_dataloader
is_valid = False    # 有些情况下的数据集并没有验证集
train_loader = get_dataloader(config, mode="train")
if is_valid:
    valid_loader = get_dataloader(config, mode="valid")
